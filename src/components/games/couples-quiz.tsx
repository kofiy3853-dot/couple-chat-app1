...
import { CheckCircle, XCircle, RotateCcw, Trophy, Share2, Users } from "lucide-react";
...
export function CouplesQuiz({ onBack, conversationId, userId, connected, socketActions, onRegisterHandlers }: CouplesQuizProps) {
  ...
  useEffect(() => {
    onRegisterHandlers("onGameChoiceMade", (data: unknown) => {
      const d = data as { fromUserId: string; fromUserName: string; game: string; payload?: { questionIndex: number; answerIndex: number } };
      if (d.fromUserId === userId || d.game !== "couples-quiz" || !d.payload) return;
      setPartnerName(d.fromUserName);
      setPartnerAnswers((prev) => ({ ...prev, [d.payload!.questionIndex]: d.payload!.answerIndex }));
      const q = shuffledQuestions[d.payload!.questionIndex];
      if (q && d.payload!.answerIndex === q.correctIndex) {
    ...
    onRegisterHandlers("onGameEnded", (data: unknown) => {
      ...
      if (d.fromUserId === userId || d.game !== "couples-quiz") return;
      setPartnerDone(true);
    });
  }, [userId, onRegisterHandlers, shuffledQuestions]);
  ...
  const handleAnswer = useCallback((index: number) => {
    ...
    if (conversationId) {
      socketActions.makeChoice(conversationId, "couples-quiz", { questionIndex: currentQuestion, answerIndex: index });
    }
  }, [selectedAnswer, question, conversationId, socketActions, currentQuestion]);

  const handleNext = () => {
    if (currentQuestion + 1 >= shuffledQuestions.length) {
      setGameOver(true);
      if (conversationId) socketActions.endGame(conversationId, "couples-quiz");
    } else {
      ...
      <GameLayout title="Couples Quiz" onBack={onBack}>
        <Card>
          <CardContent className="p-8 text-center">
            ...
            <p className="text-gray-500 mb-4">You need to be in a couple to play Couples Quiz.</p>
  ...
  if (gameOver) {
    ...
    const partnerPercentage = Math.round((partnerScore / shuffledQuestions.length) * 100);
    return (
      <GameLayout title="Couples Quiz" onBack={onBack}>
        <div className="space-y-6">
          <Card>
            <CardContent className="p-8 text-center">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-2">
                {percentage > partnerPercentage ? "You won! 🎉" : partnerPercentage > percentage ? `${partnerName || "Partner"} wins! 🎉` : "It's a tie! 🤝"}
              </h2>
              <div className="flex gap-8 justify-center mb-4">
                <div className="text-center">
                  <p className="text-sm text-gray-500">You</p>
                  <p className="text-3xl font-bold text-green-500">{score}/{shuffledQuestions.length}</p>
                  <Progress value={percentage} className="h-2 w-24 mx-auto mt-1" />
                </div>
                <div className="text-center">
                  <p className="text-sm text-gray-500">{partnerName || "Partner"}</p>
                  <p className="text-3xl font-bold text-blue-500">{partnerScore}/{shuffledQuestions.length}</p>
          ...
          <Card>
            <CardContent className="p-4">
              <p className="text-sm font-medium text-gray-500 mb-3">Compare answers</p>
              <div className="space-y-3">
                {shuffledQuestions.map((q, i) => {
                  const myA = answers[i];
                  const partnerA = partnerAnswers[i];
                  const hasPartner = partnerA !== undefined;
                  return (
                    <div key={i} className="p-3 bg-gray-50 rounded-lg text-sm">
                      <p className="font-medium text-gray-700 mb-1">{q.question}</p>
                      <div className="flex gap-4 text-xs">
                        <span className={cn(myA?.isCorrect ? "text-green-600" : "text-red-500")}>
                          You: {String.fromCharCode(65 + (myA?.selected ?? 0))} {myA?.isCorrect ? "✓" : "✗"}
                        </span>
                        {hasPartner && (
                          <span className={cn(partnerA === q.correctIndex ? "text-green-600" : "text-red-500")}>
                            {partnerName}: {String.fromCharCode(65 + partnerA)} {partnerA === q.correctIndex ? "✓" : "✗"}
                          </span>
                        )}
                        {!hasPartner && <span className="text-gray-400">{partnerName || "Partner"}: waiting...</span>}
                      </div>
                    </div>
                  );
                })}
  ...
  return (
    <GameLayout title="Couples Quiz" subtitle={`How well do you know your partner? · ${connected ? "Connected" : "Connecting..."}`} onBack={onBack}>
      <div className="space-y-6">
          ...
          <CardContent className="p-6">
            ...
            <div className="grid gap-3">
              {question.options.map((option, index) => {
                const partnerPicked = partnerAnswers[currentQuestion] === index;
                return (
                  <button
                    key={index}
